...
class Seq2seq(object):
    ...
    def attn(self, decoder_pre_state, encoder_outputs):
        # hidden: B * D
        # encoder_outputs: B * S * D
        attn_weights = tf.matmul(encoder_outputs, tf.expand_dims(decoder_pre_state, 2))
        # attn_weights: B * S * 1
        ...
        with tf.variable_scope("encoder"):
            ...
            ((encoder_fw_outputs, encoder_bw_outputs),
             (encoder_fw_final_state, encoder_bw_final_state)) = tf.nn.bidirectional_dynamic_rnn(
                ...
                sequence_length=self.seq_inputs_length, dtype=tf.float32, time_major=False)
        ...
        with tf.variable_scope("decoder"):
            ...
            def loop_fn(time, previous_output, previous_state, previous_loop_state):
                if previous_state is None:  # time step == 0
                    ...
                    return (
                    initial_elements_finished, initial_input, initial_state, initial_output, initial_loop_state)
                else:
                    def get_next_input():
